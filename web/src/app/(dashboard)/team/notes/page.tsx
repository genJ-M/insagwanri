...
export default function HrNotesPage() {
  ...
  // 노트 목록
  const { data: notes = [], isLoading, error: notesError } = useQuery<any[], any>({
  ...
  // 접근 권한 없음 (403)
  const isForbidden = notesError?.response?.status === 403;
  if (isForbidden) {
    return (
      <div className="flex-1 overflow-y-auto">
        <main className="page-container">
          <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
            ...
            <h2 className="text-lg font-semibold text-text-primary">HR 노트 열람 권한 없음</h2>
            <p className="text-sm text-text-muted max-w-sm leading-relaxed">
              HR 노트는 인사팀 관리자 또는 별도의 열람 권한을 부여받은 인원만 접근할 수 있습니다.
          ...
          <div className="md:hidden mb-4">
            <nav className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-hide">
              ...
              >
                ...
                전체
                <span className={clsx('text-[10px] px-1.5 py-0.5 rounded-full', !filterUserId ? 'bg-white/30 text-white' : 'bg-gray-100 text-gray-500')}>
                  {notes.length}
                </span>
              </button>
              {members.map((m) => {
                ...
                return (
                  ...
                  >
                    ...
                    {m.name}
                    {count > 0 && (
                      <span className={clsx('text-[10px] px-1.5 py-0.5 rounded-full', active ? 'bg-white/30 text-white' : 'bg-gray-100 text-gray-500')}>
                        {count}
                      </span>
                    )}
                  </button>
